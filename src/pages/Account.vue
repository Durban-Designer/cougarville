...
export default {
  ...
  methods: {
    updateUser () {
      let vue = this
      axios.put('https://api.cougarville.net/users/' + vue.user.id, {
    ...
    populateActiveUser () {
      let vue = this
      axios.get('https://api.cougarville.net/users/' + vue.user.id, {headers: { 'Authorization': 'JWT ' + vue.user.token }})
    ...
    registerUser () {
      let vue = this
      axios.post('https://api.cougarville.net/users', {
        email: vue.activeUser.email,
        password: vue.activeUser.password,
        name: vue.activeUser.name
      })
