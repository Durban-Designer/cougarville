...
export default {
  ...
  methods: {
    login () {
      let vue = this
      axios.post('https://api.cougarville.net/users/login', {
        ...
        .then(response => {
          if (response.status !== 401) {
            ...
            if (vue.stayLogged === true) {
              localStorage.setItem('cougarvilletoken', response.data.token)
              localStorage.setItem('cougarvilleuserId', response.data.userId)
              localStorage.setItem('cougarvillename', response.data.name)
              localStorage.setItem('cougarvilleadmin', response.data.admin)
            } else {
